...
Execution times for some input test cases-
SIZE            TIME ON (LINUX KERNEL,INTEL i5 PROCESSOR) IN SEC         TIME ON LEOPARD SERVER SYSTEM IN SEC
512                           4.4148                                             3.1831
1024                         71.6647                                            34.5535
2048                        988.9213                                           367.5124
4096                                                                          5109.9323                    
*/
...
int p_rand;//random variable used to generate input values 
void input_matrixA(char* file_name,int rows,int columns)
{
	...
	int count=0;//counter for loop generating the matrix inputs
	int buf_count=0;//to keep index count in the buffer
	p_rand=0;
	while(count<=2*(rows*columns))
	{
		if(count>0 && count%arr_size==0)
		{
			write(fd,buf,(buf_count)*sizeof(int));
			buf_count=0;
		}

		buf[buf_count]=2;//(p*p)%17;
		p_rand++;
		count++;
		buf_count++;
	}
	close(fd);
	printf("matrix initialzation completed\n");
}
...
void input_matrixB(char* file_name,int rows,int columns)
{
	...
	//directly storing the matrix in its transpose form
		...
		buf[buf_count]=3;//(p*p)%17;
		p_rand++;
		count++;
		buf_count++;
	}
	close(fd);
	printf("matrix intialization completed\n");
}

//data structure maintaining information required while loading elements into buffers from mapped pages
struct information
{
	int current_offset_A;//keeping track of element number last accessed from page number stored in page_num_A 
	int page_num_A;//starting page number mapped at a particular instance into memory
	int current_offset_B;
	int page_num_B;
	int elements_left_A;//number of elements still left to be accessed to complete the required row
}info;
...
void matrix_multiplication(int rows_mat_a,int rows_mat_b,int columns_mat_b)
{
	...
	fdc=open("matrixC",O_RDWR,0666);
	//mc=(int*)mmap(NULL,PAGE_SIZE,PROT_READ|PROT_WRITE,MAP_PRIVATE,fdc,0);
	int bufC[PAGE_SIZE/sizeof(int)]={0};
	int buf_count=0;
	int row_num_c,column_num_c,k,block_num_count;
	//printf("starting loops rowsa=%d\n",rowsa);
	for(row_num_c=0;row_num_c<rows_mat_a;row_num_c++)
	{
	//printf("outer loop\n"); 
		info.current_offset_B=0;
		info.page_num_B=0;
		...
		munmap(mat_b_ptr,PAGE_SIZE);
		//printf("cols=%d \n",cols);
		for(column_num_c=0;column_num_c<columns_mat_b;column_num_c++)
		{
			//printf("inner loop\n");
			int temp=0;//variable to store temporary result of the block
			info.elements_left_A=rows_mat_b;
			//printf("rowsb/BLOCK_SIZE=%d\n",rowsb/BLOCK_SIZE);
			for(block_num_count=0;block_num_count<=rows_mat_b/BLOCK_SIZE;block_num_count++)
			{
				...
				//printf("read block from B:y=%d of %d\n",y,rowsb/BLOCK_SIZE);
				//printf("page_num_B=%d && offset_B=%d\n",info.page_num_B,info.current_offset_B);
				int u;
			...
			if(column_num_c<columns_mat_b-1)
			{
			...
			if(buf_count==PAGE_SIZE/sizeof(int))
			{
				write(fdc,bufC,sizeof(int)*buf_count);
				buf_count=0;
			}
		}
		printf("%d\n",count);
...
int main(int argc,char* argv[])
{
	...
	//input_matrixA("matC.txt",rowsa,cols);
