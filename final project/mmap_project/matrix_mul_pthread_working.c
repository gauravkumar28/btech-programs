...
NAME:GAURAV KUMAR
Aim: relative study of parallel and sequential matrix multiplication using pthread
here we are using malloc to allocate matrix 
input: nRows,nCols,nThread
output: relative parallel and sequential run time
improvment: i faced problem in thread synchronization. i wanted to use thread in collumn as well as row it was working for small data only so i commented those part
performance was verified for  10000X10000 on i7 processor with 8GB RAM
*/
